...
exports.parseResumeFileData = exports.isResumeFile = exports.getUserParams = exports.loadSettings = exports.cutUserParamsExtraSlash = undefined;
...
var _readlineSync = __webpack_require__(6);
...
var validateUserParams = function validateUserParams(userParams) {
  return (/\/resume$/.test(userParams.resumeDir)
  );
};

// Public

var cutUserParamsExtraSlash = exports.cutUserParamsExtraSlash = function cutUserParamsExtraSlash(userParams) {
  return _extends({}, userParams, {
    resumeDir: userParams.resumeDir.replace(/\/$/, ''),
    rootPath: userParams.rootPath.replace(/\/$/, ''),
    newRootPath: userParams.newRootPath.replace(/\/$/, ''),
    newResumeDir: userParams.rewrite ? userParams.newResumeDir.replace(/\/$/, '') : null
  });
};

var loadSettings = exports.loadSettings = function loadSettings(filePath) {
  return JSON.parse(_fs2.default.readFileSync(filePath));
};

var getUserParams = exports.getUserParams = function getUserParams() {
  var resumeDir = _readlineSync2.default.question(_constants.messages.questions.resumeDir);

  var rootPath = _readlineSync2.default.question(_constants.messages.questions.rootPath);

  var newRootPath = _readlineSync2.default.question(_constants.messages.questions.newRootPath);

  var rewrite = _readlineSync2.default.question(_constants.messages.questions.rewrite) === 'y';

  var newResumeDir = null;
  if (!rewrite) {
    newResumeDir = _readlineSync2.default.question(_constants.messages.questions.newResumeDir);
  }

  return { resumeDir: resumeDir, rootPath: rootPath, newRootPath: newRootPath, rewrite: rewrite, newResumeDir: newResumeDir };
};

var isResumeFile = exports.isResumeFile = function isResumeFile(filename) {
  return (/\.resume$/.test(filename)
  );
};

var parseResumeFileData = exports.parseResumeFileData = function parseResumeFileData(fileData, userParams) {
  var params = cutUserParamsExtraSlash(userParams);
  var destinationValue = _parser2.default.getDestinationValue(fileData);
  var newDestinationValue = destinationValue.replace(params.rootPath, params.newRootPath);

  return fileData.replace(destinationValue.length + ':' + destinationValue, newDestinationValue.length + ':' + newDestinationValue);
};
...
var main = exports.main = function main(userParams) {
  ...
  _fs2.default.readdir(userParams.resumeDir, function (error, files) {
    if (error) {
      throw error;
    }
    files.map(function (filename) {
      if (utils.isResumeFile(filename)) {
        var fileData = _fs2.default.readFileSync(userParams.resumeDir + '/' + filename).toString();
        var newFileDir = userParams.rewrite ? userParams.resumeDir : userParams.newResumeDir;
        _fs2.default.writeFileSync(newFileDir + '/' + filename, utils.parseResumeFileData(fileData, userParams));
      }
    });
  });
};
...
var IS_WIN = process.platform === 'win32',
    ...
    childProc = __webpack_require__(7),
...
function _execFileSync(options, execOptions) {
  ...
  function getTempfile(name) {
    var filepath,
        suffix = '',
        fd;
    tempdir = tempdir || __webpack_require__(9).tmpdir();
...
var BencodeParser = function () {
  function BencodeParser() {
    _classCallCheck(this, BencodeParser);
  }

  _createClass(BencodeParser, null, [{
    key: 'getDestinationValue',
    value: function getDestinationValue(fileData) {
      var match = /\d+:destination/.exec(fileData);
      if (!match) {
        throw new Error(_constants.messages.errors.invalidResumeFile);
      }
      var matchEndIndex = match.index + match[0].length;
      var fileDataFromMatchIndex = fileData.slice(matchEndIndex);
      var destinationLengthString = /^\d+/.exec(fileDataFromMatchIndex)[0];
      var destinationLength = parseInt(destinationLengthString);
      return fileDataFromMatchIndex.slice(destinationLengthString.length + 1, destinationLength + destinationLengthString.length + 1);
    }
  }]);
